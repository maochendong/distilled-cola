...
把一位深耕上海房产市场的视频博主的**分析框架、推理路径、决策逻辑**完整蒸馏出来，结合 **AnySearch 实时搜索引擎**获取最新成交数据、挂牌信息和政策动态，让你随时能问出「800 万预算前滩 vs 大宁怎么选？」「上海二手房成交量 2026」这类问题，得到一个像资深分析师一样的专业回答。
...
```
         ...
         ┌──────────▼──────────┐  ┌─▼────────────────────┐
         │  OCR 流水线         │  │  转录流水线           │
         │  ingest_all.py      │  │  ingest_transcripts   │
         │  切分→标注→嵌入→清空 │  │  切分→标注→嵌入→追加  │
         └──────────┬──────────┘  └──┬──────────────────┘
                    │                │
                    └──────┬─────────┘
                           ▼
...
用户问题："800万预算前滩vs大宁怎么选？"
    ...
    │               ├──→ BM25 索引只构建一次，不随每次请求重建
    │               └──→ 查询结果 1 小时缓存（重复点击示例问题即时返回）
    │
    ├──→ 实时网络搜索（关键词自动触发）
    │       │
    │       ├──→ 查询自动追加"上海"前缀
...
│   │   ├── api.py                # FastAPI 服务（/ask, /stats, /health /static）
│   │   ├── ui.py                 # Streamlit 交互界面
│   │   └── static/               # Web UI 静态文件（index.html, logo.jpeg）                 # Streamlit 交互界面
│   └── config.py                 # 配置管理（环境变量 + 默认值）
├── data/                         # 数据存储
│   ├── raw/                      # 原始转录/OCR 结果
│   ├── processed/                # 标注后的知识块
│   ├── ocr_results/              # 帧 OCR 文本（_summary.json + .txt）
│   └── embeddings/               # ChromaDB 向量存储
├── scripts/                       # 实用脚本
│   ├── ingest_all.py              # OCR 数据完整蒸馏入库（切分→标注→嵌入→ChromaDB）
│   ├── ingest_transcriptions.py   # 转录数据追加入库（增量追加，带检查点）
│   ├── ingest_transcriptions_fast.py  # 转录快速入库（跳过标注）
│   ├── embed_and_ingest.py        # 单独嵌入+入库（从已有标注重新嵌入）
│   ├── annotate_only.py           # 单独标注（与嵌入/入库解耦）
│   ├── batch_ocr_video.py         # 批量帧 OCR（macOS Vision）
│   ├── batch_transcribe.py        # 批量 Whisper 转录
│   ├── batch_transcribe_mlx.py    # 批量 mlx-whisper 转录
│   ├── clean_ocr.py               # OCR 文本水印/伪影清理
│   └── process_videos.py          # 视频完整处理（字幕+OCR）
├── tests/                        # 单元测试
...
| 混合检索 | **RRF 融合 + 实体感知排序 + 精排** | BM25 × 语义向量，推理链匹配板块+0.15/匹配逻辑标签+0.10排序加分；Cross-encoder 二次精排，未缓存时跳过不阻塞 |
| 精排模型 | **BGE Reranker v2** | (query, doc) pair 重打分，候选池 ×4 扩召回后精排；模型未缓存时跳过不阻塞 |
...
| Web 界面 | **FastAPI StaticFiles + 自建UI** | 蓝白主题，搜索框居中布局，对话后移至底部 |
