...
将 OCR + 转录文本走完完整蒸馏链路：
**切分 → 教师模型标注 → 推理链提取 → bge-m3 向量化 → ChromaDB**

```bash
# 仅 OCR 数据先行入库
python scripts/ingest_knowledge.py --source ocr

# mlx-whisper 跑完后，融合转录+OCR 重新入库
python scripts/ingest_knowledge.py --source hybrid --reset
```
...
│   ├── ingest_knowledge.py        # 数据蒸馏入库（切分→标注→嵌入→ChromaDB）
